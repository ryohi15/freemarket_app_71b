...
mens_grandchild_array = [
                            ...
                            ['テーラードジャケット','ノーカラージャケット','Gジャン/デニムジャケット','レザージャケット','ダウンジャケット','ライダースジャケット','ミリタリージャケット','ナイロンジャケット','フライトジャケット','ダッフルコート','ピーコート','ステンカラーコート','トレンチコート','チェスターコート','モッズコート','スタジャン','スカジャン','ブルゾン','マウンテンパーカー','ダウンベスト','ポンチョ','カバーオール','その他'], 
                            ...
                            ['ショルダーバッグ','トートバッグ','ボストンバッグ','リュック/バックパック', 'ウエストポーチ', 'ボディーバッグ','ドラムバッグ','ビジネスバッグ','ドラベルバッグ','メッセンジャーバッグ','エコバッグ','その他'], 
                            ...
                            ['長財布','折り財布','マネークリップ','コインケース/小銭入れ','名刺入れ/定期入れ','キーケース','キーホルダー','ネクタイ','手袋','ハンカチ','ベルト','マフラー','ストール','バンダナ','ネックウォーマー','サスペンダー','ウォレットチェーン','サングラス/メガネ','モバイルケース/カバー','手帳','イヤマフラー','傘','レインコート/ポンチョ','ミラー','タバコグッズ','その他'], 
                            ['腕時計(アナログ)','腕時計(デジタル)','ラバーベルト','レザーベルト','金属ベルト','その他'], 
...
# ヘビー・キッズ
kids_child_array = ['ベビー服（女の子用）~95cm','ベビー服（男の子用）~95cm','ベビー服（男女兼用）~95cm','キッズ服（女の子用）100cm~','キッズ服（男の子用）100cm~','キッズ服（男女兼用）100cm~','キッズ靴','子ども用ファッション小物','おむつ/トイレ/バス','外出/移動用品','搾乳/食事','ベビー家具/寝具/室内用品', 'おもちゃ', '行事/記念品', 'その他']
kids_grandchild_array = [
                            ['トップス','アウター','パンツ','スカート','ワンピース','ベビードレス','おくるみ','下着/肌着','パジャマ','ロンバース','その他'], 
                            ['トップス','アウター','パンツ','おくるみ','下着/肌着','パジャマ','ロンバース','その他'], 
                            ['トップス','アウター','パンツ','おくるみ','下着/肌着','パジャマ','ロンバース','その他'], 
                            ['コート', 'ジャケット/上着','トップス（Tシャツ/カットソー）','トップス（トレーナー）','トップス（チェニック）','トップス（タンクトップ）','トップス（その他）','パンツ','スカート','ワンピース','セットアップ','フォーマル/ドレス','和服','パジャマ','浴衣','甚平','水着','その他'], 
                            ['コート', 'ジャケット/上着','トップス（Tシャツ/カットソー）','トップス（トレーナー）','トップス（その他）','パンツ','セットアップ','フォーマル/ドレス','和服','パジャマ','浴衣','甚平','水着','その他'], 
...
# インテリア・住まい・小物
interior_child_array = ['キッチン/食器','ベッド/マットレス','ソファ/ソファベッド','椅子/チェア','机/テーブル','収納家具','ラグ/カーペット/マット','カーテン/ブラインド','ライト/照明','寝具','インテリア小物','季節/年中行事']
interior_grandchild_array = [
                            ...
                            ['布団/毛布','枕','シーツ/カバー','その他'], 
                            ['ごみ箱','ウェルカムボード','オルゴール','クッション','クッションカバー','スリッパラック','ティッシュボックス','バスケット/かご','フォトフレーム','マガジンラック','モビール','花瓶','灰皿','傘立て'], 
                            ['正月','成人式','バレンタインデー','ひな祭り','子どもの日','母の日','父の日','サマーギフト/お中元','夏/夏休み','ハロウィン','敬老の日','七五三','お歳暮','クリスマス'] 
                          ]
...
# おもちゃ・ホビー・グッズ
hobby_child_array = ['おもちゃ・ホビー・グッズ','タレントグッズ','コミック/アニメグッズ','トレーディングカード','フィギュア', '楽器/器材', 'ミリタリー','美術品','アート用品','その他']
hobby_grandchild_array = [
                            ...
                            ['遊戯王','マジック：ザ・ギャザリング','ポケモンカードゲーム','デュエルマスターズ','バトルスピリッツ','プリパラ','アイカツ','カードファイト!! ヴァンガード','ヴァイスシュヴァルツ','プロ野球オーナーズリーグ','ベースボールヒーローズ','スリーブ','その他'], 
                            ...
                            ['画材','額縁','その他'], 
                            ['トランプ/UNO','カルタ/百人一首','ダーツ','ビリヤード','麻雀','パズル/ジグソーパズル','囲碁/将棋','オセロ/チェス','アンプ','人生ゲーム','野球/サッカーゲーム','スポーツ','三輪車/乗り物<','ヨーヨー','模型製作用品']
                          ]
...
cosme_grandchild_array = [
                            ...
                            ['アイシャドウ','口紅','リップグロス','リップライナー','チーク','フェイスカラー', 'マスカラ', 'アイライナー','つけまつげ','アイブロウペンシル','パウダーアイブロウ','眉マスカラ','トライアルセット/サンプル', 'メイク道具/化粧小物'], 
...
# 家電・スマホ・カメラ
sander_child_array = ['スマートフォン/携帯電話','スマホアクセサリー','PC/タブレット','カメラ','テレビ/映像機器','オーディオ機器','冷暖房/空調','生活家電','その他']
sander_grandchild_array = [
                            ['スマートフォン本体','バッテリー/充電器','携帯電話本体','PHS本体','その他'], 
                            ['Android用ケース','iPhone用ケース','カバー','イヤホンジャック','ストラップ','自撮り棒', 'その他'], 
...
sander_child_array.each_with_index do |child, i|
  child = parent8.children.create(name: child)
  sander_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
sport_grandchild_array = [
                            ...
                            ['ウェア','シューズ','ボール','アクセサリー','記念グッズ','応援グッズ','アクセサリー','記念グッズ','応援グッズ','その他'], 
                            ['ラケット(硬式用)','ラケット(軟式用)','ウェア','シューズ','ボール','その他'], 
...
car_grandchild_array = [
                            ...
                            ['サスペンション','ブレーキ','外装、エアロパーツ','ライト','内装品、シート','マフラー・排気系','エンジン、過給器、冷却装置','クラッチ、ミッション、駆動系','電装品','補強パーツ','汎用パーツ','外国自動車用パーツ','その他'], 
...
# その他
another_child_array = ['まとめ売り','ペット用品','食品','飲料/酒','日用品/生活雑貨/旅行','アンティーク/コレクション','文房具/事務用品','事務/店舗用品','その他']
another_grandchild_array = [
                            [], 
...
another_child_array.each_with_index do |child, i|
  child = parent13.children.create(name: child)
  another_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end